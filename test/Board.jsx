    ...
    <>
      ...
      {urlBoard.image && path2 &&
        <Card sx={{ width: "70%", marginTop: '30px', border: '1px solid lightgrey' }}>
          ...
          />
          <CardMedia component="img" height="194" image={`https://res.cloudinary.com/${process.env.REACT_APP_CLOUDINARY_CLOUD_NAME}/image/upload/${urlBoard.image[0]}`} />
          <CardContent>
          ...
          <CardActions disableSpacing>
            {/* 게시글 하단 버튼 - 좋아요 / 게시글 */}
            <Button>
              <FavoriteIcon className="customHeartBtn" />{urlBoard.likeCount}
            </Button>
            <Button onClick={() => handleOpen(urlBoard.bid)}>
              <ChatBubbleOutlineIcon className="customBtn" />{urlBoard.replyCount}
            </Button>
            <Button >
              <ShareIcon className="customBtn" />
            </Button>
            <Button>
              <BookmarkIcon className="customBtn" />
            </Button>
          </CardActions>
        </Card>
      }
      ...
      {dataList.map((data) => (
        <Card key={data.bid} sx={{ width: "70%", marginTop: '30px', border: '1px solid lightgrey' }}>
          ...
          image={`https://res.cloudinary.com/${process.env.REACT_APP_CLOUDINARY_CLOUD_NAME}/image/upload/${data.image[0]}`}
            onClick={() => handleOpen(urlBoard.bid)} sx={{ cursor: 'pointer' }} />
          <CardContent>
            <Typography variant="body2" color="text.secondary" onClick={() => handleOpen(urlBoard.bid)} sx={{ cursor: 'pointer' }}>
              {data.bContents}
            </Typography>
          </CardContent>
          <CardActions disableSpacing>
            ...
            <Button onClick={() => handleButtonLike(data.bid, data.uid)}>
              <FavoriteIcon className={`customHeartBtn ${update ? 'color' : ''}`} />{data.likeCount}
            </Button>
            <Button onClick={() => handleOpen(data.bid)}>
              <ChatBubbleOutlineIcon className="customBtn" />{data.replyCount} 
            </Button>
            <Button>
              <ShareIcon className="customBtn" />
            </Button>
                ...
                <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '62%', overflowY: 'auto' }}>
                  <Stack direction="row" spacing={1} padding={'10px 0 25px 0'}>
                    <Button sx={{ padding: 0, width: 0 }} onClick={() => handleButtonLike(board.bid, board.uid)}>
                      <FavoriteIcon className="customHeartBtn" />{board.likeCount}
                    </Button>
                      ...
                      <Paper>
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ padding: 2, overflowWrap: 'break-word', }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div>
                              {(expandedContents[index] && data.rContents != null) ? data.rContents : data.rContents.slice(0, 28)}
